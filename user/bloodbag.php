  ...
  <body>
    ...
    <div class="container page-container">
      ...
      <?php
        ...
        $sql2 = "SELECT donors.id AS donorID, donors.name, bloodbag.id, bloodbag.displayBloodbagID, bloodbag.bloodType, bloodbag.retrievedDate, bloodbag.expiryDate, bloodbag.dateCreated, bloodbag.lastEdited
          FROM donors LEFT JOIN bloodbag 
          ON donors.id = bloodbag.donorID
          WHERE bloodbag.productID = $productID AND bloodbag.status = 'In Stock'";

        $result2 = mysqli_query($con,$sql2) or die("Cannot execute sql: ".mysqli_error($con));

        $rowCount = 1;

        if(isset($_SESSION['bloodBagAdditionSuccess'])){

          echo "<div class='alert alert-success' role='alert'>".$_SESSION['bloodBagAdditionSuccess']."</div>";
          unset($_SESSION['bloodBagAdditionSuccess']);
